...
def getFitnessValue(func, chromosomesdecoded):
    # 得到种群规模和决策变量的个数
    population, _ = chromosomesdecoded.shape
    # 初始化种群的适应度值为0
...
def iterator(fitnessFunction, decisionVariables, max_iter=500):
    # 每次迭代得到的最优解
    optimalSolutions = []
    optimalValues = []
    # decisionVariables 决策变量的取值范围
    # fitnessFunction 定义适应度函数
    # 得到染色体编码长度
    ...
    for _ in range(max_iter):
        ...
        decoded = decodedChromosome(
            lengthEncode, chromosomesEncoded, decisionVariables)
        # 得到个体适应度值和个体的累积概率
        _, cum_proba = getFitnessValue(fitnessFunction, decoded)
        # 选择新的种群
        ...
        final_decoded = decodedChromosome(
            lengthEncode, mutationpopulation, decisionVariables)
        # 适应度评价
        fitnessvalues, _ = getFitnessValue(fitnessFunction, final_decoded)
...
if __name__ == '__main__':
    solution, value = iterator(lambda x: 21.5 + x[0] * np.sin(
        4 * np.pi * x[0]) + x[1] * np.sin(20 * np.pi * x[1]), [[-3.0, 12.1], [4.1, 5.8]])
    print('最优解: x1, x2:', solution[0], solution[1])
    print('最优目标函数值:', value)
