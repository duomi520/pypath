...
# 4
d.size
# 找维度0，也就是最后一个维度上的最大值，array([3, 4])
d.max(axis=0)
# 找维度1，也就是倒数第二个维度上的最大值，array([2, 4])
d.max(axis=1)
# 找维度0，也就是第一个维度上的均值，array([ 2.,  3.])
d.mean(axis=0)
# 展开一个numpy数组为1维数组，array([1, 2, 3, 4])
d.flatten()
# 展开一个可以解析的结构为1维数组，array([1, 2, 3, 4])
np.ravel(c)
...
# https://www.cnblogs.com/NSGUF/p/7406027.html
